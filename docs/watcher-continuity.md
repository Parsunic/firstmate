...
A cycle that spent itself on the recovery announcement releases the lock without republishing, so its announcement stands and the next arm supervises; that asymmetry is what bounds recovery to one cycle per down stretch.
Every watcher close and every durable queue append publishes downtime, so a downtime republication of any pending episode reuses its generation instead of minting a new one, and only a finished episode mints a fresh generation.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, an open episode that must not spend every following arm on re-announcement instead of holding the home lock, an arm inside the handling window that must leave the episode retirable by the acknowledgement already printed, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
