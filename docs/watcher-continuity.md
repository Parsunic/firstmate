...
The recovery-episode contract below owns once-per-down-stretch announcement.
...
Every supervising watcher close and every durable queue append publishes downtime, so a downtime republication of any pending episode reuses its generation instead of minting a new one, and only a finished episode mints a fresh generation.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, an open episode that must not spend every following arm on re-announcement instead of holding the home lock, an arm inside the handling window that must leave the episode retirable by the acknowledgement already printed, a cycle that discovers recovery mid-loop and must announce its generation exactly once, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
